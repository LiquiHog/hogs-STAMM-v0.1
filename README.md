...
| **Admin Contract Governance** | Permanent governor of all pools with 7-day timelocks, treasury operator role, guardian veto, and governor migration |
...
| **7-Day Admin Timelocks** | Admin contract admin transfer, treasury transfer, and governor migration use 7-day timelocks |
| **Dynamic MBR Accounting** | Factory and registry compute box funding from cached MBR rates (`set_box_mbr_rates`) |
| **Registry Admin Timelock** | Registry admin transfer uses a 72-hour timelock |
| **Factory Freeze** | Instant freeze blocks template management; unfreeze requires 7-day timelock |
...
Each pool is a standalone `STAMM` contract deployed by the `PoolFactory`. After registration, the factory transfers governor authority to the `AdminContract`, which becomes the permanent governor of all pools. Pair and LP registries live in the `RegistryContract`. Treasury claims are stored in pool state and withdrawn by the admin contract treasury role via governor-proxy methods.
...
**Copyright (c) 2026 HOG. All Rights Reserved.**

...
| **Tier P** | The passive/backstop tier (index 7) with near-zero fees and no protocol fee extraction |
...
| **Inline Spill** | Per-swap redistribution of protocol fees to Tier P (10%), the weakest tier (55%), and the 2nd weakest tier (35%). Targets determined by inline O(7) scan excluding the current tier |
...
| **Tier Mask** | A uint64 bitmask tracking which tiers are active (bit i = tier i) |
...
| **Reverse LP Registry** | Factory box storage mapping each LP asset ID to its pool, pair, and tier. Written during `register_pool_lps` |

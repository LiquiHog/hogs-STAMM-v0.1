...
| **Tier P** | The passive/backstop tier (index 6) with near-zero fees and no protocol fee extraction |
...
| **Inline Spill** | Per-swap redistribution of protocol fees to Tier P (10%), the weakest tier (55%), and the 2nd weakest tier (35%). Targets determined by inline O(6) scan of standard tiers (0-5), excluding the current tier |
...
| **Tier Mask** | A uint64 bitmask tracking which tiers are active (bit i = tier i). 7 bits used (0-6) |
...
| **Reverse LP Registry** | Factory box storage mapping each LP asset ID to its pool, pair, and tier. 7 entries per pool, written during `register_pool_lps` |
...
| **Smart-Routed Swap** | Auto-routed swap (`swap_smart`) that routes across up to 2 tiers using waterfall routing for optimal execution. Best tier fills first; overflow goes to the second-best tier |

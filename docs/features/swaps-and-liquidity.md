...
**Flow:**
1. Trader sends input tokens and provides packed routing legs: N x 9 bytes (1 byte tier index + 8 bytes amount per leg)
...
- **Bootstrap mint**: First deposit into a seeded tier (k <= bootstrap threshold). Both assets required, any ratio accepted. The depositor sets the initial price. LP tokens minted using geometric mean (`sqrt(deposit_a x deposit_b)`).

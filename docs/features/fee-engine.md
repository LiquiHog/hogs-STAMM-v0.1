...
```
                      Swap Amount
                           ...
                           ▼
                  ┌─────────────────┐
                  │  Two-Sided Fee  │  Fee charged on both input and output
                  └────────┬────────┘
                           │
              ┌────────────┴────────────┐
              │                         │
     ...
     └───────┬────────┘       └───────┬────────┘
        ...
        ▼         ▼              ▼         ▼
      80%       20%            80%       20%
   Tier LP    Protocol      Tier LP    Protocol
  (retained)    Fee        (retained)    Fee
                │                        │
                └───────────┬────────────┘
                            │
                            ▼
                   ┌─────────────────┐
                   │  Inline Spill   │  Redistribute protocol fees immediately
                   └──┬──────┬─────┬─┘
                      │      │     │
                      ▼      ▼     ▼
                    10%    55%   35%
                   Tier P   W1    W2       W1 = weakest tier (lowest k)
                  (always) (weak) (2nd)    W2 = 2nd weakest tier
                      │      │     │
                      ▼      ▼     ▼
                   ┌─────────────────┐
                   │   Mint LP for   │  LP stays in pool, tracked per tier
                   │  each recipient │  in t{c}_tl for treasury withdrawal
                   └────────┬────────┘
                            │
                       dust remainder
                            │
                            ▼
                   ┌─────────────────┐
                   │ Treasury Claims │  Withdrawn on demand by admin contract
                   │  (tr_a / tr_b)  │
                   └─────────────────┘
```
